...
1. **Detecting and Classifying Trains:** Utilizing a specialized solution involving train spot image extraction, the system identifies individual train spots on the board, classifying them as red, blue, yellow, black, green, or empty. Shown on the left, is a visual representation of the bounding boxes used to extract individual train spot images from the cropped and resized image of the game board. These bounding boxes were created using CVAT and each one is associated with a label that will allow the scoring system to determine where each space is on the game board. On the right are the extracted images, showing individual spots from a completed game. These images are cropped, rotated, then cropped again to ensure that they all have the same orientation and shape. The extracted images are classified using a simple CNN trained on over 1000 train spot images. 
...
2. **Detecting and Classifying Train Stations:** Similar to the train detection process, the system identifies train stations on cities, classifying them by color. In this step, another small CNN is used to classify the spots on the game boad where train stations would be placed. This model was trained on over 300 labelled city spot images.
